...
```js
const liftA2 = curry((f, functor1, functor2) =>
  functor1.map(f).ap(functor2)
);

const liftA3 = curry((f, functor1, functor2, functor3) =>
...
const createUser = curry(function(email, name) { /* creating... */ });
