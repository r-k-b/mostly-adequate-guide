...
const BlogController = (() => {
  const index = posts => Views.index(posts);

  const show = post => Views.show(post);

  const create = attrs => Db.create(attrs);

  const update = (post, attrs) => Db.update(post, attrs);

  const destroy = post => Db.destroy(post);
...
const validArticles = articles => articles.filter(
    article => article !== null && article !== undefined
);

// vastly more relevant for future projects
const compact = xs => xs.filter(
